...
translators.set('typescript-sync', function requireCommonJS(url, source, isMain) {
  const code = parseTScode(source);
  return FunctionPrototypeCall(translators.get('commonjs-sync'), this, url, code, false);
});
...
// Handle modules referenced by `require` calls, in typescript files
// This translator function must be sync, as `require` is sync.
translators.set('require-typescript', (url, source, isMain) => {
  const code = parseTScode(source);
  assert(cjsParse);
...
function parseTScode(source) {
  ...
  const result = parse(stringify(source), {
    mode: 'strip-only',
  });
  return result;
}
...
translators.set('typescript', async function(url, source) {
  ...
  // Try to guess if the typescript is a cts or mts
  // Depends on --experimental-detect-module to be unflagged
  // https://github.com/nodejs/node/pull/53619
  const format = containsModuleSyntax(`${code}`, fileURLToPath(url), url) ? 'module' : 'commonjs';
